...
const CareerSupport = () => {
  ...
  const handleSubmit = (e) => {
    ...
    emailjs
      .send("service_do2ikzd", "template_ls9ds3v", params, "uKqd3qDdMdOlj2TFh")
      .then(() => {
  ...
  return (
    <>
      <div className="career-support">
        ...
        <section className="career-support__queries">
            ...
            <div className="queries-container">
              {queries.map((query) => (
                <div key={query.id} className="query-card">
                  <h3>{query.name}</h3>
                  <p className="email">{query.email}</p>
                  <p className="message">{query.message}</p>
                  <p className="date">
                    {query.created_date
                      ? new Date(query.created_date).toLocaleString()
                      : "Date not available"}
                  </p>
                  <button
                    className="send-meet-btn"
                    onClick={() => handleSendMeetLink(query.email)}
                  >
                    Send Meet Link
                  </button>
                </div>
